...
gold_rewards = [[100*2, 50*2, 20*2,0,0], [100 * 4, 50 * 4, 20 * 4,0,0], [100 * 6, 50 * 6, 20 * 6,0,0]]
gem_rewards = [[10*2, 5*2, 2*2,0,0], [10 * 5, 5 * 5, 2 * 5,0,0], [10 * 10, 5 * 10, 2 * 10,0,0]]
...
enemies_stats=[[[5],[8,5],[],[8,5,0],[7,3,0]],[[7],[10,5],[],[10,5,0],[8.5,3,0]],[[8],[10,5],[],[10,5,0],[8,3,0]]]
...
while run:
    ...
    # Game screen (Car & FPS)
        ...
        console.update(mouse_pos, mouse_pressed, keys, enemys)
        if console.unlock_everything:
            inventory_display.eq=[]
            for i in items:
                inventory_display.eq.append(i.copy())
        if console.maps_unlock:
            unlocked_maps=[1 for i in unlocked_maps]
            map_snippets.unlocked_maps=unlocked_maps
            map_snippets.original_unlocks=unlocked_maps
